"""Phase III on SYN: contrast + per-shell z-score. active-site = core 0..19."""
...
print(f"pocket=32..45 ({int(labels.sum())} residues), active=0..19\n")
rows = [
...
# active-site detection from a mini raw file (GDP cofactor at the core)
print("\n--- active-site detection from raw test ---")
...
print(f" detected cofactor={code}, active-site residues={len(idx)}, "
      f"all in the core 0..19: {all(i<20 for i in idx)}")
