#!/usr/bin/env python3
"""
Quantum siec neural Under QPU (data re-uploading) in WSZYSTKICH features.
PCA(komplet cech)->6 komponentow -> QNN 3 qubits -> leave-one-out + c-Myc.
...
def _worker(arg):
    ...
    D = spaces.rank_gauss(D) # wyrownanie distributions (transfer)
    y = None
    if t.get("holo"):
...
def main():
    ...
    print(f"QNN for QPU (data re-uploading, {N_QUBITS} qubits, L={L}) in {len(names)} features\n")
    ...
    print(f"\n meana QNN_QPU={np.mean(aucs):.3f} (var wyjasniona PCA: "
          f"{PCA(n_components=N_PCA).fit(np.vstack([D[n][0] for n in tr])).explained_variance_ratio_.sum():.2f})")

    # final model in 3 proteins -> c-Myc + export circuit
    ...
    print(f"\n-> circuit QPU: {os.path.join(out, 'qnn_qpu_circuit.txt')} (Braket/Classiq)")
